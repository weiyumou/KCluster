"""Loader for ScienceQA (https://scienceqa.github.io/).

Converts the public ``problems.json`` into the Question JSONL format: text-only
multiple-choice questions with relabeled choices, keeping the dataset's
skill/subject/topic/category/grade tags as passthrough fields (``skill`` serves
as the expert KC model in the EDM 2025 evaluation).
"""
...
from kcluster.output.evaluation import compute_clustering_metrics, evaluate_random_kc
...
def write_questions(data_path: str, output_dir: str, min_choice_cnt: int = 2, min_skill_cnt: int = 100):
    ...
    dump_questions((Question(q_dict) for q_dict in sel_questions), output_path)
